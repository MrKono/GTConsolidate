...
public class MetaTileEntityTestTankA extends MultiblockWithDisplayBase {
    ...
    public MetaTileEntity createMetaTileEntity(IGregTechTileEntity tileEntity) {
        return new MetaTileEntityTestTankA(metaTileEntityId);
    }
    ...
    public long calculateCapacity(List<ITankData> tanks) {
        long capacity = 0L;
        if (!tanks.isEmpty()) {
    ...
    private static class TankBank {
        ...
        public TankBank(List<ITankData> tanks) {
            storage = new long[tanks.size()];
            maximums = new long[tanks.size()];
            for (int i = 0; i < tanks.size(); i++) {
                maximums[i] = tanks.get(i).getCapacity();
            }
            capacity = summarize(maximums);
        }

        public TankBank(NBTTagCompound storageTag) {
         ...
         */
        public TankBank rebuild(@NotNull List<ITankData> tanks) {
            if (tanks.isEmpty()) {
        ...
        private static long summarize(long[] values) {
            long retVal = 0;
            long currentSum = 0;
